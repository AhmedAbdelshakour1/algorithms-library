...
#define vec(a,b)                ((b)-(a)) // a, b complex numbers
#define same(p1,p2)             (dp(vec(p1,p2),vec(p1,p2)) < EPS)
#define dp(a,b)                 ( (conj(a)*(b)).real() )	// a*b cos(T), if zero -> either the length of one or both is 0, or the angle between them is 90 degrees.
#define cp(a,b)                 ( (conj(a)*(b)).imag() )	// a*b sin(T), if zero -> parllel
#define lengthSqrt(p)           dp(p, p) // Avoid using norm(x). Use dp(x, x) instead
#define length(a)               (hypot((a).imag(), (a).real()))
#define normalize(a)            (a)/length(a)
//#define polar(r,ang)            ((r)*exp(point(0,ang)))  ==> Already added in c++11
#define rotateO(p,ang)          ((p)*exp(point(0,ang))) //rotation about origin
#define rotateA(p,ang,about)  (rotateO(vec(about,p),ang)+about) //about arbitrary point
